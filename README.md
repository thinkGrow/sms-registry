...
- **The staff dashboard surfaces every submission still awaiting grading**, across all assessments, oldest first. "Ungraded" means no `Grade` row exists at all for that (student, assessment) pair, not just unpublished, mirroring how "Pending" is defined on the student-facing result views. There's no direct database relation between `Submission` and `Grade` (each keys independently off `studentId`/`assessmentId`), so this is a set lookup in application code rather than a join, the same pattern already used to match a submission to its grade on the assessment detail page.
