...
- **Assessment eligibility is checked without a login system.** Since role separation (staff vs. student) doesn't exist yet at this point in the build, the submission form includes a student picker rather than assuming "the current logged-in student." The eligibility rules themselves (enrolled status, matching programme) are already enforced server-side regardless of who's picking the student, so this is purely a placeholder for identifying which student is submitting, not a gap in the actual access control.
